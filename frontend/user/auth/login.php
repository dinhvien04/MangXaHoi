<div class="hb-auth-page">
    <section class="hb-auth-brand-panel">
        ...
        <div class="hb-auth-brand-copy"><span class="hb-auth-kicker">SOCIAL CAMPUS</span><h1>Kết nối gần hơn.<br>Chia sẻ dễ hơn.</h1><p>Novera Social là không gian để bạn đăng bài, theo dõi bạn bè, trò chuyện và cập nhật những khoảnh khắc quan trọng mỗi ngày.</p></div>
        <div class="hb-auth-social-preview"><div class="hb-preview-avatar">QC</div><div><strong>Trần Quỳnh Chi</strong><small>Vừa xong · Công khai</small></div><p>Hello Novera!</p><span>Một nơi nhỏ để kết nối với nhau.</span></div>
    </section>
    <section class="hb-auth-form-panel">
        <form class="hb-auth-card" method="post" action="?action=login">
            ...
            <p class="hb-auth-subtitle">Đăng nhập để tiếp tục với Novera Social.</p>
            <?php if (isset($_GET['next']) && $_GET['next'] === 'admin'): ?>
                <div class="hb-alert hb-alert-info"><i class="bi bi-shield-lock-fill"></i><span>Hãy đăng nhập bằng tài khoản có vai trò <strong>Admin</strong>. Hệ thống sẽ tự chuyển vào Novera Control Center.</span></div>
            <?php endif; ?>
            ...
            <div class="hb-auth-security"><i class="bi bi-shield-check"></i><span>Phiên đăng nhập được bảo vệ và quyền tài khoản được kiểm tra ở backend.</span></div>
        </form>
    </section>
</div>

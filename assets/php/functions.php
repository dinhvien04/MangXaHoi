...
function gettime($date)
{
    ...
    $weekday = $weekdays[date('l', $timestamp)]; // Lấy ngày trong tuần bằng tiếng Việt
...
function followUser($user_id)
{
    ...
    createNotification($cu['id'], $user_id, "đã bắt đầu theo dõi bạn!");
    return mysqli_query($db, $query);
}
...
function blockUser($blocked_user_id)
{
    ...
    createNotification($cu['id'], $blocked_user_id, "đã chặn bạn");
...
function unblockUser($user_id)
{
    ...
    createNotification($current_user, $user_id, "đã bỏ chặn bạn!");
    return mysqli_query($db, $query);
}
...
function like($post_id)
{
    ...
    if ($poster_id != $current_user) {
        createNotification($current_user, $poster_id, "thích bài viết của bạn!", $post_id);
    }
...
function addComment($post_id, $comment)
{
    ...
    if ($poster_id != $current_user) {
        createNotification($current_user, $poster_id, "đã bình luận về bài đăng của bạn", $post_id);
    }
...
function unlike($post_id)
{
    ...
    $poster_id = getPosterId($post_id);
    if ($poster_id != $current_user) {
        createNotification($current_user, $poster_id, "đã bỏ thích bài đăng của bạn!", $post_id);
    }
...
function unfollowUser($user_id)
{
    ...
    createNotification($current_user, $user_id, "đã bỏ theo dõi bạn!");
    return mysqli_query($db, $query);
}
...
function validateSignupForm($form_data)
{
    ...
    if (!$form_data['email']) {
        $response['msg'] = "VUi lòng nhập email";
        $response['status'] = false;
        $response['field'] = 'email';
    }

    if (!$form_data['last_name']) {
        $response['msg'] = "Vui lòng nhập họ";
        $response['status'] = false;
        $response['field'] = 'last_name';
    }
    if (!$form_data['first_name']) {
        $response['msg'] = "Vui lòng nhập tên";
        $response['status'] = false;
        $response['field'] = 'first_name';
    }
    if (isEmailRegistered($form_data['email'])) {
        $response['msg'] = "email đã được đăng ký";
        $response['status'] = false;
        $response['field'] = 'email';
    }
    if (isUsernameRegistered($form_data['username'])) {
        $response['msg'] = "username đã được đăng kí";
        $response['status'] = false;
        $response['field'] = 'username';
    }
...
function validateLoginForm($form_data)
{
    ...
    if (!$form_data['username_email']) {
        $response['msg'] = "Vui lòng nhập username/email";
        $response['status'] = false;
    ...
    if (!$blank && !checkUser($form_data)['status']) {
        $response['msg'] = "có gì đó không đúng, chúng tôi không thể tìm thấy bạn";
        $response['status'] = false;
...
//     return $data;
// }
// KIểm tra quyền hạn user khi đăng nhập
function checkUser($login_data)
{
    global $db;
    $username_email = $login_data['username_email'];//vấn đề
    $password = $login_data['password'];
    $role = $login_data['role'];
    
    // Thêm điều kiện kiểm tra role là 'Admin'
    $query = " SELECT * FROM users WHERE (email='$username_email' || username='$username_email') AND role='User' ";
    $run = mysqli_query($db, $query);
...
function createUser($data)
{
    ...
    $password_text= md5($password);
    $role = $data['role'];
    $query = "INSERT INTO users(first_name,last_name,gender,email,username,password, password_text,role) ";
    $query .= "VALUES ('$first_name','$last_name',$gender,'$email','$username','$password',' $password_text','$role')";
    return mysqli_query($db, $query);
}
...
function validateUpdateForm($form_data, $image_data)
{
    ...
    if ($image_data['name']) {
        ...
        if ($type != 'jpg' && $type != 'jpeg' && $type != 'png') {
            $response['msg'] = "chỉ cho phép hình ảnh jpg,jpeg,png";
            $response['status'] = false;
            $response['field'] = 'profile_pic';
        }

        if ($size > 1000) {
            $response['msg'] = "tải lên hình ảnh có kích thước nhỏ hơn 1 mb";
            $response['status'] = false;
            $response['field'] = 'profile_pic';
        }
    }

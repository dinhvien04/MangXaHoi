...
<div class="container col-md-9 col-sm-11 rounded-0">
            ...
            <div class="d-flex flex-column">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#chatbox"
                                        onclick="popchat(<?= $profile['id'] ?>)"><i class="bi bi-chat-fill"></i> Nhắn tin</a>
                                </li>
                                <li><a class="dropdown-item "
                                        href="assets/php/actions.php?block=<?= $profile['id'] ?>&username=<?= $profile['username'] ?>"><i
                                            class="bi bi-x-circle-fill"></i> Chặn</a></li>
                            </ul>
                        </div>
                    <?php
                    }
                ...
                ?>
                    <div class="d-flex gap-2 align-items-center my-3">
                        ...
                        <a class="btn btn-sm btn-primary"><i class="bi bi-file-post-fill"></i> <?= count($profile_post) ?> bài
                            đăng</a>
                        <a class="btn btn-sm btn-primary <?= count($profile['followers']) < 1 ? 'disabled' : '' ?>"
                            ...
                            <?= count($profile['followers']) ?> người theo dõi</a>
                        <a class="btn btn-sm btn-primary <?= count($profile['following']) < 1 ? 'disabled' : '' ?>"
                            ...
                            <?= count($profile['following']) ?> người đang theo dõi</a>
                ...
                ?>
                    <div class="d-flex gap-2 align-items-center my-1">
                        ...
                        ?>
                            <button class="btn btn-sm btn-danger unblockbtn" data-user-id='<?= $profile['id'] ?>'>Mở chặn</button>

                        <?php
                        } else if (checkBlockStatus($profile['id'], $user['id'])) { ?>
                            <div class="alert alert-danger" role="alert">
                                <i class="bi bi-x-octagon-fill"></i> @<?= $profile['username'] ?> Đã chặn bạn!
                            </div>
                        <?php } else if (checkFollowStatus($profile['id'])) {
                        ?>
                            <button class="btn btn-sm btn-danger unfollowbtn" data-user-id='<?= $profile['id'] ?>'>Bỏ theo
                                dõi</button>

                        <?php
                        } else {
                        ?>
                            <button class="btn btn-sm btn-primary followbtn" data-user-id='<?= $profile['id'] ?>'>Theo
                                dõi</button>
    ...
    ?>
        <div class="alert alert-secondary text-center" role="alert">
            <i class="bi bi-x-octagon-fill"></i> Bạn không được phép xem bài viết!
        </div>
    <?php

    } else if (count($profile_post) < 1) {
        echo "<p class='p-2 bg-white border rounded text-center my-3'>Bạn không có bài viết nào</p>";
    }
    ?>
                            ...
                            <div class="col-md-4 col-sm-12 d-flex flex-column">
                                ...
                                ?>
                                    <div class="input-group p-2 border-top">
                                        <input type="text" class="form-control rounded-0 border-0 comment-input"
                                            placeholder="nói điều gì đó.." aria-label="Recipient's username"
                                ...
                                ?>
                                    <div class="text-center p-2">
                                        Nếu bạn muốn bình luận hãy theo dõi người dùng này !</div>
